...
use archetype_ecs::hot_reload::{HotReloadApp, ReloadableSystem};
use archetype_ecs::{App, System, SystemAccess, World};
...
impl System for MovementSystem {
    ...
    fn run(
        ...
        println!(
            "🎮 Running MovementSystem (counter: {}, speed: {:.2}, last_reload: {:?})",
...
impl ReloadableSystem for MovementSystem {
    fn reload(&mut self) -> Result<(), archetype_ecs::error::EcsError> {
        ...
        println!(
            "✅ MovementSystem reloaded successfully! New speed multiplier: {:.2}",
...
impl System for RenderSystem {
    ...
    fn run(
        ...
        println!(
            "🎨 Running RenderSystem (counter: {}, mode: {}, last_reload: {:?})",
            self.counter, self.render_mode, self.last_reload
        );

        let entity_count = world.query::<&Position>().iter().count();
        println!(
            "  Rendering {} entities in {} mode",
            entity_count, self.render_mode
        );
...
impl ReloadableSystem for RenderSystem {
    fn reload(&mut self) -> Result<(), archetype_ecs::error::EcsError> {
        ...
        println!(
            "✅ RenderSystem reloaded successfully! New render mode: {}",
            self.render_mode
        );
        Ok(())
    }
...
fn main() {
    ...
    println!(
        "Check interval: {:?}",
        app.hot_reload_manager().check_interval
    );
