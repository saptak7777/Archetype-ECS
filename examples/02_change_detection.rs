...
fn main() {
    ...
    for (pos, _vel) in world.query_mut::<(&mut Position, &Velocity)>().iter().take(50) {
        pos.x += 1.0;
        modified_count += 1;
    }
    ...
    println!("  - {} entities remain unchanged", world.entity_count() - modified_count);
    ...
    println!("\nWithout change detection: Process {} entities every frame", world.entity_count());
